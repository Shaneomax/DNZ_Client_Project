    ...
    public class DrawMesh : MonoBehaviour
    {
        ...
        public void EndDraw(Vector2 centOfMass)
        {
            ...
            else
            {
                ...
                gameObject.AddComponent<FreezeMoving>();
            }
        ...
        {
            switch (drawSettings.drawMeshMode)
            {
                case DrawMeshMode.Mode2D:
                    CalculateVerticesAndUV2D();
                    CalculateTriangles2D();
                    break;

                case DrawMeshMode.Mode3D:
        ...
        #region Normal2D
        private void CalculateVerticesAndUV2D()
        {
            vertices.Clear();
            uvs.Clear();
            ...
            for (int i = 0; i < line; i++)
            {
                vertices.Add(new Vector3(points[points.Count - 1 - i].x, points[points.Count - 1 - i].y, 0));
                uvs.Add(GetUVWithLerp(i, true));
            }
        }

        private void CalculateTriangles2D()
        {
            triangles.Clear();
...
#if UNITY_EDITOR
        /// <summary>
        /// Display the center of gravity of the selected object.
        /// </summary>
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawSphere(rigidbody2d.worldCenterOfMass, 0.2f);
        }
#endif
    }
}